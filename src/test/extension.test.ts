...
test("NetCDFTreeProvider shows attributes for variables", async () => {
  ...
  const attrsNode = timeChildren.find(
    (item: any) => item.label === "Attributes",
  );
  ...
  const dataVarsNode = roots.find(
    (item: any) => item.label === "Data Variables",
  );
  ...
  const tempAttrsNode = tempChildren.find(
    (item: any) => item.label === "Attributes",
  );
